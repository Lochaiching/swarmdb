...
func (t *Table) OpenTable(u *SWARMDBUser) (err error) {
	...
	if len(roothash) == 0 {
		return &SWARMDBError{message: fmt.Sprintf("[swarmdb:OpenTable] Empty root hash"), ErrorCode: 403, ErrorMessage: fmt.Sprintf("Table Does Not Exist: TableName [%s] Owner [%s]", t.tableName, t.Owner)}
	}
	setprimary := false
...
func (t *Table) DescribeTable() (tblInfo map[string]Column, err error) {
	...
	for cname, c := range t.columns {
		...
		if _, ok := tblInfo[cname]; ok { // if ok, would mean for some reason there are two cols named the same thing
			return tblInfo, &SWARMDBError{message: fmt.Sprintf("[swarmdb:DescribeTable] Duplicate column: [%s]", cname), ErrorCode: -1, ErrorMessage: "Table has Duplicate columns?"} //TODO: how would this occur?
		}
		tblInfo[cname] = cinfo
	}
