...
// var fs = require('fs');
// var swarmdbinfo = require('./../swarm.json');
...
Node.prototype.getStats = function(forced)
{
	...
	readLastLine.read('/tmp/netstats.log', 1)
		.then((line) => {
			// console.log(line);
			var swarmdbinfo = JSON.parse(line);

			self.stats.storage.NodeID = swarmdbinfo.NodeID;
			self.stats.storage.WalletAddress = swarmdbinfo.WalletAddress;

			if (swarmdbinfo.ChunkStat) {
				self.stats.storage.ChunkStat.ChunkR = parseInt(swarmdbinfo.ChunkStat.ChunkR);
				self.stats.storage.ChunkStat.ChunkRL = parseInt(swarmdbinfo.ChunkStat.ChunkRL);
				self.stats.storage.ChunkStat.ChunkS = parseInt(swarmdbinfo.ChunkStat.ChunkS);
				self.stats.storage.ChunkStat.ChunkSL = parseInt(swarmdbinfo.ChunkStat.ChunkSL);
				self.stats.storage.ChunkStat.ChunkW = parseInt(swarmdbinfo.ChunkStat.ChunkW);
				self.stats.storage.ChunkStat.ChunkWL = parseInt(swarmdbinfo.ChunkStat.ChunkWL);
			}



			self.stats.storage.LaunchDT = swarmdbinfo.LaunchDT;
			self.stats.storage.LReadDT = swarmdbinfo.LReadDT;
			self.stats.storage.LWriteDT = swarmdbinfo.LWriteDT;
			self.stats.storage.LogDT = swarmdbinfo.LogDT;


			self.stats.bandwidth.sNodeID = swarmdbinfo.NodeID;
			self.stats.bandwidth.sWalletAddress = swarmdbinfo.WalletAddress;

			if (swarmdbinfo.SStat) {
				self.stats.bandwidth.SStat.SwapI = parseInt(swarmdbinfo.SStat.SwapI);
				self.stats.bandwidth.SStat.SwapIA = parseInt(swarmdbinfo.SStat.SwapIA);
				self.stats.bandwidth.SStat.SwapIAL = parseInt(swarmdbinfo.SStat.SwapIAL);
				self.stats.bandwidth.SStat.SwapIL = parseInt(swarmdbinfo.SStat.SwapIL);
				self.stats.bandwidth.SStat.SwapR = parseInt(swarmdbinfo.SStat.SwapR);
				self.stats.bandwidth.SStat.SwapRA = parseInt(swarmdbinfo.SStat.SwapRA);
				self.stats.bandwidth.SStat.SwapRAL = parseInt(swarmdbinfo.SStat.SwapRAL);
				self.stats.bandwidth.SStat.SwapRL = parseInt(swarmdbinfo.SStat.SwapRL);
			}
